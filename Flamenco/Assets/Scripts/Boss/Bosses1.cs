...
{  
    ...
    public enum State
    { // enums are (very basically) integers with names.
        Tackle,
        IDLE,
        ATTACK_1,
        Rock
    ...
    public State state = State.Tackle;
    ...
    void FixedUpdate()
    {
        ...
        if (FinalFight.iniciaFinal == true)
        {
            ...
            if (tiempo > 10)
            {
                ...
                if (Suerte == 3 || DescansoObligado >= 3)
                {
                    state = State.IDLE; 
                }
            }
    ...
    void Embestidas()
    {
        ...
        Invoke("Caida", 2f);
        body.AddForce(((Vector2.up +(Vector2.right * direction) ) * Salto));
        Debug.Log("E");
    }
    void Caida()
    {
